...
const BottomNav = ({ activeTab, setActiveTab, filters, setFilters, onBackToHome, requireLogin, onShowSubmitModal }) => (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 h-16 flex items-center justify-around z-40 pb-safe">
        <button onClick={() => { setActiveTab('home'); setFilters({ ...filters, selectedCompany: null }); onBackToHome(); }} className={`flex flex-col items-center gap-1 ${activeTab === 'home' ? 'text-indigo-600' : 'text-slate-400'}`}>
            <Home size={24} />
            <span className="text-[10px] font-medium">首頁</span>
        </button>
        <button onClick={() => !requireLogin() && onShowSubmitModal()} className="flex flex-col items-center gap-1 text-slate-400">
